...
| `cpu_isa` | Highest x86-64 psABI microarchitecture feature level supported by all online PUs, e.g. `x86-64-v3`. The resource remains a `string_array`, but the hook publishes exactly one level. Empty on non-x86-64 systems. | `select=1:cpu_isa=x86-64-v3` |
| `cpu_spec` | Site-defined floating-point relative performance value for one CPU core, selected from `cpu_spec_map`. | Node property for scheduler/site policy. |
...
`cpu_isa` publishes only the highest supported level. A node whose highest
detected level is `x86-64-v3` therefore publishes only `x86-64-v3`. Although
the PBS resource type remains `string_array`, the hook intentionally supplies
a single value.
...
- On x86-64, `cpu_isa` is the highest fully supported level. Detection starts from the `x86-64-v1` baseline and advances only while every required feature of each successive level is available.
...
9. On x86-64, derives the highest supported `cpu_isa` level from the common CPU flags.
10. Matches the resulting CPU model against `cpu_spec_map`; the first matching entry wins, otherwise `default_cpu_spec` is used.
...
The resource remains defined as `string_array`, but the hook publishes only
the highest supported level. For example, a v4 node publishes:

```text
x86-64-v4
```

This makes `cpu_isa` a classification of the node's maximum common ISA level,
rather than a cumulative capability list. Consequently, a request for
`cpu_isa=x86-64-v3` does not also match a node that publishes only
`x86-64-v4`; minimum-ISA semantics would require separate policy logic.
...
`cpu_spec_map` is evaluated in list order and uses shell-style wildcard matching against the consolidated `cpu_model` string. The first matching entry provides the floating-point `cpu_spec` value. If no entry matches, `default_cpu_spec` is used.
...
| `default_cpu_spec` | float | `0.0` | Fallback `cpu_spec` value when no CPU model mapping matches. The supplied configuration uses `30.0`. |
| `cpu_spec_map` | array | `[]` | Ordered mapping of CPU model patterns to per-core `cpu_spec` values. |
...
| `cpu_spec_map[].value` | float | — | Value published as `resources_available.cpu_spec`. |
...
- `cpu_spec` mapping is string-pattern based and is a site policy, not a hardware benchmark performed by the hook.
