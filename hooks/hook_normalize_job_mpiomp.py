...
* npus_per_core is a vnode string property used for an exact match on uniform
...
def main():
    ...
    if "user_select" not in job.Resource_List.keys():
        job.Resource_List["user_select"] = old_select

    new_select = normalize_select(old_select)

    log(pbs.EVENT_DEBUG, "original select: %s" %
        str(job.Resource_List["user_select"]))
